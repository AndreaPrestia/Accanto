...
#   3. (se ci sono credenziali) login + endpoint autenticato funzionano end-to-end
#
# Esce con codice 0 se tutti i check passano, 1 al primo errore.
#
# Uso:
#   deploy/smoke.sh <BASE_APP_URL> [BASE_WEB_URL]
#
# Esempio:
...
# Crealo una volta sola con una POST a /api/auth/register e archivia le credenziali
...
if [[ -z "$APP_URL" ]]; then
  echo "usage: $0 <BASE_APP_URL> [BASE_WEB_URL]" >&2
  exit 2
fi
...
echo "▶ Smoke test contro $APP_URL"

# ------------------------------------------------------------------
# 1. Health readiness (processo + DB)
# ------------------------------------------------------------------
ready_body="$(curl -fsS --max-time 10 "$APP_URL/health/ready" || true)"
...
# ------------------------------------------------------------------
# 2. Sito vetrina (opzionale, solo se passi BASE_WEB_URL)
# ------------------------------------------------------------------
if [[ -n "$WEB_URL" ]]; then
...
# ------------------------------------------------------------------
# 3. Login + endpoint autenticato (solo se credenziali smoke fornite)
# ------------------------------------------------------------------
if [[ -n "${SMOKE_EMAIL:-}" && -n "${SMOKE_PASSWORD:-}" ]]; then
  ...
  login_body="$(curl -fsS --max-time 10 -H 'Content-Type: application/json' \
    -d "$login_payload" "$APP_URL/api/auth/login" || true)"
  ...
  me_body="$(curl -fsS --max-time 10 -H "Authorization: Bearer $token" "$APP_URL/api/auth/me" || true)"
  me_email="$(echo "$me_body" | jq -r '.email // empty')"
  [[ "$me_email" == "$SMOKE_EMAIL" ]] || fail "/api/auth/me email mismatch ($me_email != $SMOKE_EMAIL)"
  pass "/api/auth/me → $me_email"

  circles_code="$(curl -fsS -o /dev/null -w '%{http_code}' --max-time 10 \
    -H "Authorization: Bearer $token" "$APP_URL/api/care-circles" || true)"
  [[ "$circles_code" == "200" ]] || fail "/api/care-circles ha restituito HTTP $circles_code"
  pass "/api/care-circles → 200"
else
