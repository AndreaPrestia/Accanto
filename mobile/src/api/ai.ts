...
export async function setCircleAiEnabled(
  circleId: string,
  enabled: boolean
): Promise<void> {
...
export async function timelineSummary(
  ...
  const { data } = await api.post<AiResponse>(
    `/care-circles/${circleId}/ai/timeline-summary`,
    { days }
  );
  return data;
}

export async function doctorQuestionDraft(
  circleId: string,
  topic: string,
  notes?: string
): Promise<AiResponse> {
  const { data } = await api.post<AiResponse>(
    `/care-circles/${circleId}/ai/doctor-question-draft`,
    { topic, notes }
  );
  return data;
}
...
export async function rephrase(
  ...
  const { data } = await api.post<AiResponse>(
    `/care-circles/${circleId}/ai/rephrase`,
    { text, tone }
  );
  return data;
}
...
export async function checkInReflection(days = 14): Promise<AiResponse> {
  const { data } = await api.post<AiResponse>('/me/ai/checkin-reflection', {
    days
  });
  return data;
}
...
export async function listAiInteractions(
  ...
  const { data } = await api.get<AiInteractionListResponse>('/ai/interactions', {
    params
  });
  return data;
}
