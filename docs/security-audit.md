...
32. **Dual-write documenti su S3-compatible (IONOS) via outbox** ✅ Fatto il 2026-06-19.
    ...
    sincrona-logica/asincrona-fisica su un bucket S3 compatibile
    **dedicato** (`accanto-docs` IONOS Francoforte, prefix
    `documents/`, Versioning ON, **Object Lock OFF** per restare
    compatibili con la cancellazione GDPR art. 17). Bucket
    intenzionalmente separato da `accanto-backups` per due motivi:
    (a) blast radius — la key dell'app NON deve esporre i dump DB
    se compromessa; (b) policy bucket-level esplicita — Object Lock
    enabled su `accanto-backups`, disabled su `accanto-docs`, niente
    config ibrida ambigua.
...
| 2026-06-19 | main post-s3-dual-write | `document_sync_outbox` + `DocumentSyncWorker` (BackgroundService) + `S3DocumentReplica` (PutAsync blob cifrato, DeleteAllVersionsAsync su bucket versionato). Outbox enqueue nella stessa transazione di `DocumentService.Upload/Delete` → DB e S3 coerenti. Gating completo via `S3DocumentReplica:Enabled` (default off, zero deps in dev/test). Bucket IONOS **dedicato** `accanto-docs` (prefix `documents/`, Versioning ON, Object Lock OFF — GDPR-compatibile), separato da `accanto-backups` (Object Lock GOVERNANCE 7y) per ridurre blast radius della key applicativa. Test 171/171 PASS (2 nuovi `DocumentSyncOutboxTests`) + CLI `accanto smoke-s3` per validare endpoint/credenziali end-to-end prima dell'attivazione in prod. | Da single-point-of-failure (disk locale) a copia geografica cifrata best-effort. RPO ≈ 10s + worker latency. Chiave cifratura resta solo lato app → bucket leak non decifra nulla. |
